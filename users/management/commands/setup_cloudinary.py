        ...
        if cloud_name and api_key and api_secret:
            try:
                ...
                if result.get('status') == 'ok':
                    self.stdout.write(
                        self.style.SUCCESS('✅ Cloudinary connection test successful!'))
                else:
                    self.stdout.write(
                        self.style.ERROR('❌ Cloudinary connection test failed'))
            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(
                        f'❌ Cloudinary connection error: {
                            str(e)}'))

        self.stdout.write('')
        self.stdout.write('Image transformations configured:')
        self.stdout.write(
            '• Profile pictures: Auto-optimized, face detection, rounded corners')
        self.stdout.write(
            '• Build images: Auto-optimized, multiple sizes available')
        self.stdout.write(
            '• Automatic format selection (WebP, AVIF when supported)')
        self.stdout.write('• Quality optimization based on content type')
        self.stdout.write('')
        self.stdout.write(self.style.SUCCESS(
            '🚀 Ready to use Cloudinary for image management!'))
